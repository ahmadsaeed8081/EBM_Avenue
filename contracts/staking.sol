...
contract EBM_Avenue_Staking
    {
     ...
     mapping(address=>bool) public isUser;
        ...
        function BuyAndStake(address _add,uint _investedamount,uint choose_val) public  returns(bool success)
        {
            require(msg.sender==buy_contract);
            ...
            if(user[_add].investBefore == false)
            { 
                All_investors[totalusers]=_add;
                isUser[_add]=true;

                totalusers++;                                     
            }

            uint num = user[_add].noOfInvestment;
            user[_add].investment[num].investedAmount =_investedamount;
            user[_add].investment[num].DepositTime=block.timestamp;
            user[_add].investment[num].withdrawnTime=block.timestamp + details[choose_val].timeframe ;  
            
            user[_add].investment[num].investmentNum=num;
            user[_add].investment[num].apr=details[choose_val].APR;
             user[_add].investment[num].timeframe=(details[choose_val].timeframe/per_day_divider) ;  


            user[_add].totalInvestment+=_investedamount;
            user[_add].noOfInvestment++;
            totalbusiness+=_investedamount;


            user_investments[_add][num] = user[_add].investment[num];
        ...
        function update_buyToken(address _add)  public
        {
            require(msg.sender==owner,"only Owner can call this function");
            buy_contract = _add;
        }
